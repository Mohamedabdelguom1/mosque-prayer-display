...
function serviceWorkerPrecache(): Plugin {
  return {
    ...
    closeBundle() {
      ...
      /*
       * صور المكتبة تُستثنى من التخزين المسبق.
       *
       * الشاشة تُقلع بالصور الست الافتراضية، اما بقية المكتبة فقد لا
       * يختار المشرف منها شيئا اصلا. لو خزّنّاها كلها مسبقا لأضفنا نحو
       * اربعة ميغابايت الى اول اقلاع بلا طائل، وهو اقلاع يجري غالبا
       * على شبكة المسجد بعد انقطاع كهرباء. وعامل الخدمة يخزّن ايّ صورة
       * منها عند اول عرض لها، فتبقى متاحة بلا انترنت بعد ذلك.
       */
      const precache = files
        .filter((f) => f !== 'sw.js' && !f.startsWith('bg/library/'))
        .sort();
